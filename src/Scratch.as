...
public class Scratch extends Sprite {
	...
	public var canSave:Boolean = true;
	public var canSaveTolocal:Boolean = true;
	public var canRemix:Boolean = true;

	public var ProjectServer:String = 'http://localhost:2345';
	public var AssetServer:String = 'http://localhost:2345';
	...
	private var topBarOnline:TopBarPart;
	...
	protected function initialize():void {
		...
		canSave = (loaderInfo.parameters['canSave'] == 'true');
		canSaveTolocal = (loaderInfo.parameters['canSaveTolocal'] == 'true');
		canRemix = (loaderInfo.parameters['canRemix'] == 'true');
	...
	protected function setupExternalInterface(oldWebsitePlayer:Boolean):void {
		if (!jsEnabled) return;
		//ExternalInterface.addCallback("ASsetEditMode",app.setEditMode);
		...
		ExternalInterface.addCallback("ASloadProject",this.LoadProject);
		//addExternalCallback("ASloadProject",app.LoadProject);
		ExternalInterface.addCallback("ASsetEditMode",this.setEditMode);
		ExternalInterface.addCallback("ASrightMouseDown",this.gh.rightMouseDown);
		...
		ExternalInterface.addCallback("ASTitle",this.projectName);
	...
	public function reportSaveFailure() : void
	{
		...
		//DialogBox.notify('Upload failed','network not connecting.\n Trying again in {sec}...')
	}
	...
	{
		Server.getInstance().setProjectThumbnail(param1,param2,param3);
	}
	...
	protected function saveProject(param1:Boolean, param2:Function = null) : void
	{
		...
		uploadSucceeded = function(param1:String):void
		{
			...
			app.logMessage(LogLevel.INFO,"Project saved!");
//			handleSaveResponse(param1);
			server.setProjectThumbnail(projectID,projectThumbnail,thumbnailSaved);
		...
		{
			//this.topBar.showTransitionNotice("Remixing...");
		}
		else if(this.copyRequested)
		{
			//this.topBar.showTransitionNotice("Copying...");
		}
		saveStartTime = CachedTimer.getCachedTimer();
		oldProjectID = projectID;
		projectThumbnail = stagePane.projectThumbnailPNG();
		new ProjectIO(this).uploadProject(stagePane,projectID,this.remixRequested || this.copyRequested,uploadSucceeded);
...
//		}
	...
	private function shouldSave() : Boolean
      {
         return saveNeeded && editMode && canSave;
      }
	...
	public function okayToAdd(newAssetBytes:int):Boolean {
		...
		if (assetByteCount > assetByteLimit) {
			...
			DialogBox.notify(
					'非常抱歉！',
					'添加此媒体会超出' + overBy + ' KB大小的限制\n' +
					'在添加其他媒体之前，请删除一些角色，背景或者声音',
					stage);
			return false;
		}
		return true;
	...
	public function LoadProject(id:String,title:String):void{
        ...
        {
            var pio:ProjectIO = new ProjectIO(app);
            pio.downloadProjectAssets(loader.data);
            app.setProjectName(title);
        });
		loader.addEventListener(ProgressEvent.PROGRESS,function(event:ProgressEvent):void
        {
            lp.setProgress(event.bytesLoaded / event.bytesTotal);
        });
		app.projectID = id;
		url = app.AssetServer + "/GetProject.php?id=" + app.projectID + "&v=" + new Date().getTime();
		var request:URLRequest = new URLRequest(url);
	...
	public function Upload(url:String,data:ByteArray,done:Function):void{
		var request:URLRequest = new URLRequest(url);
		request.method = URLRequestMethod.POST; 
		var variables:URLVariables = new URLVariables(); 
		request.data = data;
